...
```{r import libraries}
req_packages <- c("DBI", "RMySQL", "dplyr", "dbplyr", "knitr", "tidyr", "readr", "stringr", "tibble", "rmarkdown", "purrr", "lubridate", "here", "httr2", "ggplot2", "RCurl", "mice", "naniar", "gtExtras", "minqa", "patchwork", "forcats", "tools", "janitor", "skimr", "scales", "viridis", "corrplot", "broom", "pROC", "kableExtra","tinytex")
...
```
**Import .CSV data into MYSQL Database - (Paula Brown)**
...
watch_history_clean_data_DB <- dbReadTable(databaseconnect,"watch_history")
watch_history_clean_data_DB$is_download <- as.logical(watch_history_clean_data_DB$is_download)
watch_history_clean_data_DB$watch_date <- as.Date(watch_history_clean_data_DB$watch_date)

recommendation_log_clean_data_DB <- dbReadTable(databaseconnect,"recommendation_log")
recommendation_log_clean_data_DB$was_clicked <- as.logical(recommendation_log_clean_data_DB$was_clicked)
recommendation_log_clean_data_DB$recommendation_date <- as.Date(recommendation_log_clean_data_DB$recommendation_date)

users_clean_data_DB <- dbReadTable(databaseconnect,"users")
users_clean_data_DB$subscription_start_date <- as.Date(users_clean_data_DB$subscription_start_date)

movies_clean_data_DB <- dbReadTable(databaseconnect,"movies")
movies_clean_data_DB$is_netflix_original <- as.logical(movies_clean_data_DB$is_netflix_original)
movies_clean_data_DB$content_warning <- as.logical(movies_clean_data_DB$content_warning)
movies_clean_data_DB$added_to_platform <- as.Date(movies_clean_data_DB$added_to_platform)
...
This section loads the data from the MYSQL database.

```{r load-cleaned, message=FALSE}

movies <- movies_clean_data_DB %>% as_tibble()
users  <- users_clean_data_DB  %>% as_tibble()
watch  <- watch_history_clean_data_DB  %>% as_tibble()
rec    <- recommendation_log_clean_data_DB  %>% as_tibble()
...
**Transform, clean, analyze, and visualize recommended movie data - (Paula Brown)**

Glimpse the newly created dataframes (user_movie_history and user_recommended_movies) to confirm data is present.
...
1. Removing remaining NA values from data.
...
head(user_movie_history)
head(user_movie_history_trans)
```

2. Changing 0 and 1 values in the "is_netflix_original", "is_active", and "is_download" fields to "No" and "Yes", respectively.
...
## Select necessary data from the transformed dataframe

1. Begin a new frame to explore each user's watched movies, and ratings. 
*The goal is to determine:
  a.) whether any of the users in the recommendation_log data-set appear based on the global baseline estimate formula or something else
  b.) whether the users and movies in my newly created recommendation log (based on the global baseline formula) matches the one listed in the original recommendation_log data-set.

# Modify the cases in movie titles and the user's name
...
head(user_movie_history_ls) #Preview first 6 rows of data
```
## User Average
...
```
## Join Predicted_Rating to user_movie_history_ls
```{r predicted_rating_to_ls}
user_movie_history_ls <- user_movie_history_ls %>%
  mutate(Predicted_Rating = Global_Average + User_Bias + Movie_Bias)
```
#View a few rows of data in the user_movie_history_ls dataframe after averages and predicted ratings have been added
```{r}
head(user_movie_history_ls) #Preview with averages and predicted ratings added
```
## Create a pivoted dataframe to display predicted movie ratings for each user
...
pivot_predicted_ratings <- user_movie_history_ls %>%
  ...
  pivot_wider(
    ...
    values_fill = list(Predicted_Rating = "")
  )
head(pivot_predicted_ratings) #Preview the data
```
List of user recommended movies
```{r limited_set_rm}
...
#user_recommended_movies_ls
```
#Pivot data in the user_recommended_movie_ls dataframe (original recommendation_log data set) to view user recommendations and recommended scores for each user
```{r}
original_recommendation <- user_recommended_movies_ls %>%
  mutate(Movie_Label = paste0(Movie_Title, " (", Movie_ID, ")"),
         Recommendation_Score = as.character(Recommendation_Score)) %>%
  distinct(User_ID, Movie_Label, .keep_all = TRUE) %>%
  select(User_ID, Movie_Label, Recommendation_Score) %>%
  pivot_wider(
    names_from = Movie_Label,
    values_from = Recommendation_Score,
    values_fill = list(Recommendation_Score = "")
  )
head(original_recommendation)
```

## Turning pivot_predicted_ratings' pivot wide data to pivot long to get top 5 recommended
```{r long_predicted}
...
```
## Top 5 predicted recommendations
```{r top_five}
top_5_recommendations <- long_predicted %>%
  ...
  ungroup()
```
## Top 5 Matrix for predicted recommendations
```{r top_five_matrix}
...
## Turning original_recommendation pivot wide data to pivot long to get top 5 recommended
```{r long_original_rec}
long_original_rec <- original_recommendation %>%
  pivot_longer(
    cols = -c(User_ID),
    names_to = "Movie_Label",
    values_to = "Recommendation_Score"
  )
```
## Top 5 recommendations
```{r top_five_orig_rec}
top_5_orig_recommendations <- long_original_rec%>%
  filter(!is.na(Recommendation_Score)) %>%
  group_by(User_ID) %>%
  slice_max(order_by = Recommendation_Score, n = 5, with_ties = FALSE) %>%
  arrange(User_ID, desc(Recommendation_Score)) %>%
  ungroup()
```
## Top 5 Matrix
```{r top_five_matrix_orig_rec}
top_5_matrix_orig_rec <- top_5_orig_recommendations %>%
  group_by(User_ID) %>%
  mutate(rank = paste0("Top_", row_number())) %>%
  pivot_wider(names_from = rank, values_from = Movie_Label)
head(top_5_matrix_orig_rec)
```

# Reshape recommendations for comparison
```{r}
long_model <- top_5_matrix %>%
  pivot_longer(cols = starts_with("Top_"), names_to = "Rank", values_to = "Recommended_Movie")
long_model <- long_model %>% distinct(User_ID, Rank, .keep_all = TRUE)
```
# Reshape original recommendations for comparison
```{r}
long_orig <- top_5_matrix_orig_rec %>%
  pivot_longer(cols = starts_with("Top_"), names_to = "Rank", values_to = "Original_Movie")
long_orig  <- long_orig  %>% distinct(User_ID, Rank, .keep_all = TRUE)
```
## Begin comparing top 5 recommended movies to top 5 recommended movies from original movie data set.
```{r compare_top_5_recs}
comparison_df <- full_join(long_model, long_orig, by = c("User_ID", "Rank")) %>%
  mutate(Match = case_when(
    is.na(Recommended_Movie) | is.na(Original_Movie) ~ "No",
    Recommended_Movie == Original_Movie ~ "Yes",
    TRUE ~ "No"
  ))
```
Comparison summary
```{r summarize_comparison}
match_summary <- comparison_df %>%
  count(Match) %>%
  mutate(Percent = round((n / sum(n))*100))
```
## Plot top 5 comparison percentage
```{r ggplot_comparison_df}
ggplot(match_summary, aes(x = Match, y = Percent, fill = Match)) +
  geom_col() +
  labs(title = "Ranked Movie Match Accuracy", x = "Match", y = "Percent") +
  scale_fill_manual(values = c("Yes" = "skyblue", "No" = "violet")) +
  scale_y_continuous(labels = scales::percent_format(scale = .0001)) +
  theme_minimal()
```
None of the Top 5 recommended movies matched the original recommended movies. Specifically, no matches were found at the Top 1 rank. For ranks Top 2 through Top 5, one or both data frames contained NA values, indicating that no valid comparison could be made for those positions across users.

## Start by extracting relevant entries from the recommendation_log to evaluate whether the predicted movie ratings correspond to the movies that were recommended to each user.
1. Predicted movie in the recommendation log?
```{r}
## Extract the Movie_ID from the Movie_Title in the predicted movies (long_predicted) df
predicted_movies <- long_predicted %>%
  #filter(!is.na(Predicted_Rating)) %>%
  filter(Predicted_Rating!="") %>%
  mutate(Movie_ID = str_extract(Movie_Label, "\\(([^)]+)\\)")) %>%
  mutate(Movie_ID = str_remove_all(Movie_ID, "[()]"))

## Now checking for match
matched_recommendations <- user_recommended_movies_ls %>%
  left_join(predicted_movies %>% select(User_ID, Movie_ID, Predicted_Rating), 
            by = c("User_ID", "Movie_ID")) %>%
  mutate(Match = if_else(!is.na(Predicted_Rating), "Yes", "No")) %>%
  select(User_ID, Movie_ID, Movie_Title, Match)
#top_5_recommendations
head(matched_recommendations)
```

```{r calc_percent}
calc_match_percent <- matched_recommendations %>%
  count(Match) %>%
  mutate(Percent = round((n / sum(n)) * 100, 1))
print(calc_match_percent)
```

```{r plot_matches}
ggplot(calc_match_percent, aes(x = Match, y = Percent, fill = Match)) +
  geom_col() +
  labs(title = "Predicted Movies Found in Recommendation Log", 
       x = "Match Status", y = "Percent of Recommendations") +
  scale_fill_manual(values = c("Yes" = "skyblue", "No" = "violet")) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  theme_minimal()
```
```{r config_predicted_movies_for_match }
Pm <- predicted_movies %>% filter(!is.na(Predicted_Rating))
Pm <- Pm %>% filter(Predicted_Rating != "")
```
## Percentage of data in the recommendation_log that matches the data in the top 5 matrix
```{r percent_match}
matched1 <- user_recommended_movies_ls %>%
  inner_join(Pm %>% select(User_ID, Movie_ID), by = c("User_ID", "Movie_ID"))
total_recs <- nrow(user_recommended_movies_ls)
matched_recs <- nrow(matched1)
percent_matched <- (matched_recs / total_recs) * 100
print(percent_matched)
```

It appears as though the users and associated movies listed in the recommendation_log may not have been recommended based on the global baseline prediction formula. ~0.9% of recommendations in the recommendation_log were in the predicted_movies data frame (which was calculated using the global baseline formula), ~99.1% was not.
